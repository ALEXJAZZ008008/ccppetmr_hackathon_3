...
def googlenet_preoutput_module(x):
...
def googlenet_output_module(x):
    x = googlenet_preoutput_module(x)
...
def resnet_conv_module(x, conv_filter_1, conv_filter_2, conv_filter_3, conv_kernal, conv_stride):
    ...
    x_shortcut = k.layers.Dropout(0.25)(x_shortcut)
    x_shortcut = k.layers.Activation("relu")(x_shortcut)
